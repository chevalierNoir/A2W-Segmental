...
class FrameCountSampler(tud.Sampler):
    def __init__(self, shuffle, batch_size, files, lengths, seeds=list(range(100))):
        if not isinstance(shuffle, bool):
            raise ValueError("shuffle should be a boolean value,"
                             "but got shuffle={}"
                             .format(shuffle))
        ...
        self.idx2len = {}
        for ix, f in enumerate(files):
            fidx[f].append([ix, lengths[ix][0]])
            self.idx2len[ix] = lengths[ix][0]
        self.fidx = list(fidx.values())
        logging.info("Initialize seeds and index array")
        seed = seeds.pop()
        batch_indexes = self.make_batches(self.fidx, seed, shuffle)
        self.state_dict_ = {'seeds': seeds, 'batch_indexes': batch_indexes}
    ...
    def __iter__(self):
        if len(self.state_dict_['batch_indexes']) == 0:
            seed = self.state_dict_['seeds'].pop() if len(self.state_dict_['seeds']) > 0 else 222
            logging.info(f"Making new batch from new seed {seed}")
            self.state_dict_['batch_indexes'] = self.make_batches(self.fidx, seed, self.shuffle)
        while True:
            if len(self.state_dict_['batch_indexes']) > 0:
                batch = self.state_dict_['batch_indexes'].pop()
                yield batch
    ...
    def make_batches(self, file_indexes, seed, shuffle):
        perm = []
        batch_indexes = []
        if shuffle:
            np.random.seed(seed)
            ixs = np.random.permutation(len(file_indexes))
        else:
            ixs = np.arange(len(file_indexes))
        for ix in ixs:
            sorted_indexes = sorted(file_indexes[ix], key=lambda x: x[1])
            perm.append(sorted_indexes)
        for index_group in perm:
            batch_group = []
            accum_batch_size, batch = 0, []
            for index, ilen in index_group:
                if accum_batch_size + ilen > self.batch_size:
                    batch_group.append(batch)
                    accum_batch_size, batch = 0, []
                batch.append(index)
                accum_batch_size += ilen
            if len(batch) > 0:
                batch_group.append(batch)
            np.random.shuffle(batch_group)
            batch_indexes.extend(batch_group)
        return batch_indexes
